...
#pragma warning restore 649
        ...
        private void Update()
        {
            ...
            foreach (var arm in arms)
            {
                if (Input.GetKeyDown((arm.GetSiblingIndex()+1).ToString()))
                {
                    ...
                    {
                        foreach (var activeArm in arms)
                        {
                            if (activeArm.gameObject.activeInHierarchy)
                            {
        ...
        private void RotateCameraAndCharacter()
        {
            foreach (var arm in arms)
            {
                ...
                var clampedY = RestrictVerticalRotation(rotationY, arm);
        ...
        private float RestrictVerticalRotation(float mouseY, Transform arm)
        {
